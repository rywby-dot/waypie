...
class Configurator(Gtk.Application):
    ...
    def do_activate(self):
        ...
        hint = Gtk.Label(
            label=(
                ...
                "Select a submenu and press “Open submenu” to edit its children."
            ),
            xalign=0,
            wrap=True,
        )
    ...
    def on_property_changed(self, _widget):
        ...
        if self.selected_path:
            item.angle = round(self.angle_spin.get_value()) % 360
            item.x = None
            item.y = None
        row = self.path_rows.get(self.selected_path)
        if row is not None:
    ...
    def on_preview_click(self, _gesture, _presses, x, y):
        self.select_preview_item(self.preview_hit(x, y))
    ...
    def on_drag_update(self, _gesture, offset_x, offset_y):
        ...
        item = self.item_at((*self.current_path, self.drag_index))
        item.angle = round(direction_angle(x - center_x, y - center_y)) % 360
        item.x = None
        item.y = None
        self.sync_fields()
        self.canvas.queue_draw()
